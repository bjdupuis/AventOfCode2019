...
fun calculateWirePath(path: String): Map<Pair<Int,Int>,Int> {
    ...
    for (move in moves) {
        ...
        when (direction) {
            'R' -> {
                for (travel in 1..distance) {
                    steps++
                    val pair = Pair(x+travel, y)
                    if (!wire.contains(pair)) {
                        wire[pair] = steps
                    }
                }
                x += distance
