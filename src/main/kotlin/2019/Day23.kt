...
class Day23(input: String) {
    ...
    class Packet(val x: Long, val y: Long)
    private val robotCount = 50

    private var finalPacket : Packet? = null

    enum class State {
        AWAITING_ADDRESS, AWAITING_X, AWAITING_Y;
    }

    class BrainState {
        var state: State = State.AWAITING_ADDRESS
        var x: Long = -1L
        var y: Long = -1L
        var address: Long = -1L
    }

    private val brainStates = Array<BrainState>(robotCount) { BrainState() }

    private val initialRegisters = mutableListOf<Long>().apply {
        input.split(",")
    ...
    private val brains = mutableListOf<IntCode>().apply {
        for (index in 0 until robotCount) {
            val brain = IntCode(initialRegisters, "Brain-$index")
            add(brain)
        }
    }

    fun calculatePart1Answer(): Long {
        val countDownLatch = CountDownLatch(1)
        for (index in 0 until robotCount) {
            val brain = brains[index]
            compositeDisposable.add(brain.awaitingInputSubject
                .delay(1L, TimeUnit.SECONDS)
                .subscribe { brain.inputSubject.onNext(-1) })
            compositeDisposable.add(brain.outputSubject.subscribe { output ->

                println("${brain.name} sent $output")

                val brainState = brainStates[index]
                when (brainState.state) {
                    State.AWAITING_ADDRESS -> {
                        brainState.address = output
                        brainState.state = State.AWAITING_X
                    }
                    State.AWAITING_X -> {
                        brainState.x = output
                        brainState.state = State.AWAITING_Y
                    }
                    State.AWAITING_Y -> {
                        brainState.y = output
                        brainState.state = State.AWAITING_ADDRESS

                        println("${brain.name} sending (${brainState.x},${brainState.y}) to ${brainState.address}")

                        if (brainState.address == 255L) {
                            finalPacket = Packet(brainState.x, brainState.y)
                            countDownLatch.countDown()
                        } else {
                            val destinationBrain = brains[brainState.address.toInt()]
                            destinationBrain.inputSubject.onNext(brainState.x)
                            destinationBrain.inputSubject.onNext(brainState.y)
                        }
                    }

                }
            })
            brain.inputSubject.onNext(index.toLong())
        }

        countDownLatch.await()
        compositeDisposable.clear()

        return finalPacket!!.y
    }

    fun calculatePart2Answer(): Long {
        val lock = Object()
        val idleBrains = ConcurrentSkipListSet<Int>()
        var natState : Pair<Long, Long>? = null
        val natYValuesSent = mutableSetOf<Long>()
        val countDownLatch = CountDownLatch(1)
        var answer = 0L
        for (index in 0 until robotCount) {
            val brain = brains[index]
            idleBrains.add(index)
            compositeDisposable.add(brain.awaitingInputSubject
                .delay(100L, TimeUnit.MILLISECONDS)
                .subscribe {
                    synchronized(lock) {
                        brain.inputSubject.onNext(-1)
                        idleBrains.add(index)
                        println("${brain.name} idle, total is ${idleBrains.size}")
                        if (idleBrains.size == robotCount && natState != null) {
                            val wake = brains[0]
                            wake.inputSubject.onNext(natState!!.first)
                            wake.inputSubject.onNext(natState!!.second)
                            if (natYValuesSent.contains(natState!!.second)) {
                                countDownLatch.countDown()
                                answer = natState!!.second
                            }
                            natYValuesSent.add(natState!!.second)
                        }
                    }

                })

            compositeDisposable.add(brain.outputSubject
                .subscribe { output ->
                    synchronized(lock) {
                        idleBrains.remove(index)
                        println("${brain.name} sent $output")
                        ...
                        when (brainState.state) {
                            ...
                            State.AWAITING_Y -> {
                                ...
                                if (brainState.address == 255L) {
                                    natState = Pair(brainState.x, brainState.y)
                                } else {
                                    val destinationBrain = brains[brainState.address.toInt()]
                                    destinationBrain.inputSubject.onNext(brainState.x)
                                    destinationBrain.inputSubject.onNext(brainState.y)
