...
fun main(args: Array<String>) {
    val input1 = """409	194	207	470	178	454	235	333	511	103	474	293	525	372	408	428
...
3858	202	1141	3458	2507	239	199	4400	3713	3980	4170	227	3968	1688	4352	4168"""
    ...
    println("Day 2 part 1 solution is " + day2.calculateChecksum(input1))
}
...
class Day2 {
    fun calculateChecksum(input: String): Int {
        ...
        for (line in input.lines()) {
            val strings = line.split("\t".toRegex())
            val values = strings.map { it.toInt() }

            for (first in 0 until values.size - 1) {
                val firstValue = values[first]
                for (second in first + 1 until values.size) {
                    val secondValue = values[second]
                    sum = when(max(firstValue, secondValue) % min(firstValue, secondValue)) {
                        0 -> sum + (max(firstValue, secondValue) / min(firstValue, secondValue))
                        else -> sum
                    }
                }
            }
