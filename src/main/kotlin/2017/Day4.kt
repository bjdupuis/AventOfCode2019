...
juo pmiyoh xxk myphio ogfyf dovlmwm moevao qqxidn 
    ...
    for (line in lines) {
        ...
        val sorted = words.map { word -> word.chars().sorted().toArray().map { it.toChar() }.joinToString("")}
        if (words.size == words.distinct().size && words.size == sorted.distinct().size) {
            totalValid++
        }
    }

    println("$totalValid valid passphrases")
}